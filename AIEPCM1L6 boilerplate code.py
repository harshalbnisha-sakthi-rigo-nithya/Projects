...
def list_genres(df):
    return sorted(set(genre.strip()) for sublist in df['Genre'].values for genre in sublist)

genres = list_genres(movies_df)

# Recommend movies based on filters (genre, mood, rating)
def recommended_movies(genre = None, mood = None, rating = None, top_n = 5):
...
def display_recommendations(recs, name):
    print(Fore.YELLOW + f"\nAi=I-Analyzed Movie Recommendations for {name}:")
